...
from io_gen.check import validate_flattened

from tests.utils import *

def test_flatten_matches_expected_fixture():
    with open("tests/fixtures/valid/pipeline_input.yaml") as f:
    ...
    with open("tests/fixtures/valid/expected_flatten_signals.json") as f:
        expected = json.load(f)

    sorted_result = normalize_dicts(sorted(result, key=sort_key))
    sorted_expected = normalize_dicts(sorted(expected, key=sort_key))
    validate_flattened(sorted_expected)
    validate_flattened(sorted_result)
